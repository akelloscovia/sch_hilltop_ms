...
### Automatic migration & seed helper

For convenience there are helper scripts in `mgnt/scripts`:

- `run_migrations.py` — runs Alembic migrations using `DATABASE_URL` from your environment or `.env`.
- `seed_db.py` — seeds initial website content and ensures default contact/home/about records.
- `migrate_and_seed.py` — runs migrations then seeds the DB.

Usage (from repo root):
```bash
# Ensure .env is configured (copy from mgnt/.env.sample)
cd mgnt
python -m venv .venv
