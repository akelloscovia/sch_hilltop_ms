...
# Import all models early to register them with SQLAlchemy
from app.models import (
    Role, User, Student, StudentParent, StudentDocument, StudentTransfer,
    PromotionHistory, Class, Subject, TimetableEntry, ClassNote,
    FeePayment, Expense, Book, BorrowRecord, BusRoute, BoardingAssignment,
    Dormitory, DormitoryAllocation, Attendance, AttendanceStatus, Marks,
    TermReport, Message, Announcement, ContactInfo, ContactMessage,
...
def create_app(config_name=None):
    ...
    # Wrap in a short retry loop to tolerate transient DB/network unavailability during deploy.
    def init_db_with_retries(max_retries=6, delay=5):
        attempts = 0
        while True:
            try:
                db.create_all()
                ...
                if attempts > max_retries:
                    # Give up and re-raise so the failure is visible in logs
                    raise
                print(f"Database not ready (attempt {attempts}/{max_retries}): {e}")
                time.sleep(delay)
